...
namespace cme {
  ...
  void ChannelController::OnIncrementalPacket(Packet *packet) {
    ...
    if (seqnum > expectedseqnum_) {
      std::cout << "Packet level seq gap detected on incremental channel "
                << expectedseqnum_ << " > " << seqnum
                << ", ignoring unless RptSeq sync issue." << '\n';
    }
  }
  ...
  void ChannelController::Subscribe(Instrument instrument) {
    auto inst_controller = GetInstrumentController(instrument.securityid);
    if (inst_controller == nullptr) {
      instrument_controllers_.emplace(instrument.securityid, std::move(InstrumentController(instrument, channel_)));
    }
  }
